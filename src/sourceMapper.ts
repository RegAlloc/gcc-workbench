...
export class SourceMapper {
    ...
    constructor(private context: vscode.ExtensionContext) {
        this.rainbowDecorations = rainbowPalette.map(color => 
            vscode.window.createTextEditorDecorationType({ backgroundColor: color, isWholeLine: true })
        );
    }

    public async enable(activeEditor: vscode.TextEditor) {
        ...
        if (rootPath) {
            ...
            if (fs.existsSync(absolutePath)) sourceUri = vscode.Uri.file(absolutePath);
        }
        if (!sourceUri) {
            const rtlDir = path.dirname(rtlDoc.uri.fsPath);
            const relativeToRtl = path.resolve(rtlDir, rawPath);
            if (fs.existsSync(relativeToRtl)) sourceUri = vscode.Uri.file(relativeToRtl);
        }
        if (!sourceUri) {
            const filename = path.basename(rawPath);
            const files = await vscode.workspace.findFiles(`**/${filename}`, '**/node_modules/**', 1);
            if (files.length > 0) sourceUri = files[0];
        }
...
// ⚡️ HARDCORE MODE: ZERO-ALLOCATION PARSER
    ...
    private buildMapping(text: string) {
            ...
            if (balance > 0 && blockSourceLine === -1) {
                ...
                if (c === CHAR_COLON && i > 0 && text.charCodeAt(i - 1) === CHAR_QUOTE) {
                    ...
                    while (p < len) {
                        ...
                        if (digit < CHAR_0 || digit > CHAR_9) break;
                        hasDigits = true;
                        p++;
                    }
